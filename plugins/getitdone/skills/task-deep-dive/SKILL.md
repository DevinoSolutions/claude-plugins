...
- `get_task_details`: the task's rich-text notes, every attachment with a short-lived URL, and its dependency chain (what blocks it and what it blocks).
- `get_task_video_context`: evenly spaced still frames from a video uploaded to the task's notes or attachments, returned as images with a text summary, plus any YouTube or Vimeo links found in the notes.
